		...
		for i in range(len(population)):
			for j in range(i+1, len(population)):
				...
				for k in range(repetition):
					'''
					i_play = int(population[i].play(population[j].type))
					j_play = int(population[j].play(population[i].type))
					population[i].observe(j_play, population[j].type)
					population[j].observe(i_play, population[i].type)
					...
					A_i = np.matmul(np.transpose(population[i].action_matrix[population[j].get_type()]), P_i)
					A_j = np.matmul(np.transpose(population[j].action_matrix[population[i].get_type()]), P_j)
					...
					TA_i = sum([population[i].get_submatrix(population[j].get_type(), k) * A_i[k] for k in range(2)])
					TA_j = sum([population[j].get_submatrix(population[i].get_type(), k) * A_j[k] for k in range(2)])
					
					
					P_i = np.matmul(TA_i, P_i) 
					P_j = np.matmul(TA_j, P_j) 
...
Pops = Population(12,2, repetition, fresh_mind=True)
